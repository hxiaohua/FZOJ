...
<td class="{sorter:'false'}" width=5%>Rank
<th width=10%>User</th>
<th width=10%>Name</th>
<th width=5%>Solved</th>
<th width=5%>Penalty</th>
<?php
	$sql_ques="select problem.title 
	from problem,homework_problem 
	where problem.problem_id=homework_problem.problem_id and homework_id='$hid'
	order by problem.problem_id;";
	$re_ques=mysqli_query($mysqli,$sql_ques);
for ($i=0;$i<$pid_cnt;$i++){
	//echo "<td><a href=problem.php?hid=$hid&pid=$i>$PID[$i]</a></td>";原始输出ABCD
	 $quesrow=mysqli_fetch_row($re_ques);
	 echo "<td><a href=problem.php?hid=$hid&pid=$i>$PID[$i].$quesrow[0]</a></td>";
...
$uuid=$U[$i]->user_id;
$nick=$U[$i]->nick;
//昵称需要修改为用户名字，真实姓名,新建函数查找，没找到就返回nick
if($nick[0]!="*")
echo $rank++;
else
echo "*";
...
//上面输出的是真实的nick，或者名字
echo "<td><a href=status.php?user_id=$uuid&hid=$hid>$usolved</a>";
