...
<?php
	...
	from problem,contest_problem 
	where problem.problem_id=contest_problem.problem_id and contest_id='$cid'
	order by problem.problem_id;";
	$re_ques=mysqli_query($mysqli,$sql_ques);
for ($i=0;$i<$pid_cnt;$i++){
	//echo "<td><a href=problem.php?cid=$cid&pid=$i>$PID[$i]</a></td>";原始输出ABCD
	 $quesrow=mysqli_fetch_row($re_ques);
	 echo "<td><a href=problem.php?cid=$cid&pid=$i><font color='white'>$PID[$i].$quesrow[0]</font></a></td>";
