...
if ( isset( $_GET[ 'hid' ] ) ) {
	...
	*/
	...
	//PYB增加,私有题目，判断用户是否在列表中；
	if ( $view_private ) {

		$sql = "SELECT * FROM `privilege` WHERE `rightstr`='h$hid' and `user_id`='$uid'";
		$result = mysqli_query( $mysqli, $sql );
		$myre = mysqli_query( $mysqli, $sql );
		$myrow = mysqli_fetch_assoc( $myre );
...
//无hid参数，则输出homework列表
	...
	//  echo "$myhomeworks";
	//页码设置增加页面
	$sql="SELECT COUNT(*) as num from homework";
	$result = mysqli_query( $mysqli, $sql );
	$row = mysqli_fetch_object( $result );
	$page_num=intval($row->num);
	$view_total_page=$page_num/100+1;
	$page_str="limit 0,100";//sql limit限制数量用，限制每页100个
	if(isset($_GET['page']))
	{
		$page=intval($_GET['page']);
		$start_num=($page-1)*100;//将100修改即可改成每页的数值
		$page_str="limit $start_num,100";
		//echo $page_str;
	}
	//add by hxh end
	$wheremy = "";
	...
	$sql = "SELECT * FROM `homework` WHERE `defunct`='N' and homework.title like '%$keyword%' ORDER BY `homework_id` DESC $page_str"; //此处删除limit 0,1000
	//$sql = "select *  from homework left join (select * from privilege where rightstr like 'k%') p on concat('k',homework_id)=rightstr where homework.defunct='N' and homework.title like '%$keyword%' $wheremy  order by homework_id desc $page_str;"; //删除限制limit 0,1000，最新添加的将会显示在最前面
	$result = mysqli_query( $mysqli, $sql );
	$view_homework = Array();
	$i = 0;
	while ( $row = mysqli_fetch_object( $result ) ) {
		...
		$view_homework[ $i ][ 6 ] = "admin"; //不显示谁创建的
		$i++;
	}
	mysqli_free_result( $result );
}
