...
//	echo $endtime;
   ...
   foreach($lang as $t){
	...
	if (count($pieces)>0 && strlen($pieces[0])>0){
		for ($i=0;$i<count($pieces);$i++)
		{
			$sql_1="INSERT INTO `username` (`user_id`, `name`) 
			VALUES ('".trim($pieces[$i])."','".trim($stuname[$i])."');";
			echo "<br />".$sql_1."<br />";
			if(!mysqli_query($mysqli,$sql_1))//插入不成功，选择更新
			{
				$sql_1="UPDATE `username` 
    ...
    */
	...
	(select privilege.user_id,users.name from privilege left join users on privilege.user_id=users.user_id WHERE `rightstr`='c$cid' order by user_id)as Tabname
	";
	$result=mysqli_query($mysqli,$sql) or die(mysqli_error($mysqli));
	for ($i=mysqli_num_rows($result);$i>0;$i--){
		$row=mysqli_fetch_row($result);
