...
$sql="SELECT
        users.user_id,users.nick,solution.result,solution.num,unix_timestamp(solution.in_date)-$start_time in_date
                FROM
                        (select * from solution where solution.homework_id='$hid' and num>=0  and problem_id>0) solution
                left join users
...
$sql="select num,user_id from
        (select num,user_id from solution where homework_id=$hid and result=4 order by solution_id ) homework
        group by num";
if($OJ_MEMCACHE){
//        require("./include/memcache.php");
...
/////////////////////////Template
require("template/".$OJ_TEMPLATE."/homeworkrank2.php");
