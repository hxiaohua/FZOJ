...
//重写checkac，只涉及作业模块，是否重判等。
function check_isac($hid,$pid){
	//require_once("./include/db_info.inc.php");
	$mysqli=$GLOBALS['mysqli'];
	//首先，判断作业是否需要重判,默认设置不需要哈
	$sql="SELECT * FROM `homework` WHERE `homework_id`='$hid' and `recheck`='Y'";
	$result=mysqli_query($mysqli,$sql);
	$rows_cnt=mysqli_num_rows($result);
	if($rows_cnt)
	$sql="SELECT count(*) FROM `solution` WHERE `problem_id`=
	(select problem_id from homework_problem where `homework_id`='$hid' AND `num`='$pid')
	AND `result`='4' AND `homework_id`='$hid' AND `user_id`='".$_SESSION['user_id']."'";
	else
	$sql="SELECT count(*) FROM `solution` WHERE `problem_id`=
	(select problem_id from homework_problem where `homework_id`='$hid' AND `num`='$pid')
	AND `result`='4' AND `user_id`='".$_SESSION['user_id']."'";
	$result=mysqli_query($mysqli,$sql);
	$row=mysqli_fetch_array($result);
	$ac=intval($row[0]);
	mysqli_free_result($result);
...
//检测作业中的某一题是否完成
function check_ok($uid,$pid,$hid,$recheck){
	$mysqli=$GLOBALS['mysqli'];
	$sql="SELECT count(*) FROM `solution` WHERE `problem_id`='$pid'
	AND `result`='4' AND `user_id`='$uid'";
	if($recheck=='Y')
	$sql="SELECT count(*) FROM `solution` WHERE `problem_id`='$pid'
	AND `result`='4' AND `homework_id`='$hid' AND `user_id`='$uid'";
	$result=mysqli_query($mysqli,$sql);
	$row=mysqli_fetch_array($result);
	$ac=intval($row[0]);
	mysqli_free_result($result);
